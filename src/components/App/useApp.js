...
export const useApp = () => {
  const savedLocation =
    JSON.parse(localStorage.getItem("chosenLocation")) || {};
  const savedTempUnit = JSON.parse(localStorage.getItem("tempUnit"));
  const savedChosenLocationWeather = JSON.parse(
    localStorage.getItem("chosenLocationWeather")
  );

  const [chosenLocation, setChosenLocation] = useState(savedLocation);
  const [tempUnit, setTempUnit] = useState(savedTempUnit);
  const [chosenLocationWeather, setChosenLocationWeather] = useState(
    savedChosenLocationWeather
  );

  const [fetchError, setFetchError] = useState("");
  const [isLoadingCurrentAndForecast, setIsLoadingCurrentAndForecast] =
    useState(false);

  const fetchCurrentAndForecastWeather = useCallback(
    async (chosenLocation, tempUnit) => {
